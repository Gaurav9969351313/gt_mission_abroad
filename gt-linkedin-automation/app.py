...
def execute_function(func_name, params):
    ...
    try:
        if func_name in FUNCTION_MAP:
            FUNCTION_MAP[func_name](params)  # Pass params as a single argument
        else:
...
def load_jobs_from_config():
    """Load jobs from config.json and insert into the database with dynamic scheduling."""
    try:
        ...
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM jobs")  # Remove all existing jobs
        
        current_time = datetime.datetime.now()
        logging.info(f"Application started at {current_time}. Scheduling jobs dynamically.")
        
        for index, job in enumerate(config_data):
            function_name = "scrapper"
            parameters = json.dumps(job['parameters'])
            
            # Calculate dynamic cron schedule with 5-minute intervals
            dynamic_time = current_time + datetime.timedelta(minutes=index * 5)
            cron_expression = f"{dynamic_time.minute} {dynamic_time.hour} * * *"
            
            cursor.execute("INSERT INTO jobs (function_name, parameters, cron_expression) VALUES (?, ?, ?)",
                           (function_name, parameters, cron_expression))
        conn.commit()
        conn.close()
        logging.info("Jobs loaded from config.json and scheduled dynamically successfully.")
    except Exception as e:
        logging.error(f"Error loading jobs from config.json: {e}")
...
@app.route("/trigger_scrapper_all", methods=["GET"])
def trigger_scrapper_all():
    """API to trigger the scrapper function based on job id from config.json"""
    try:
        with open('config.json', 'r') as config_file:
            config_data = json.load(config_file)['data']
        ...
        for job in config_data:
            if job:
                scrapper(job['parameters'])
                logging.info(f"Scrapper triggered successfully for job id {job['id']}.")
                return jsonify({"message": f"Scrapper triggered successfully for job id {job['id']}"}), 200
            else:
                logging.warning(f"Job id {job['id']} not found in config.json.")
                return jsonify({"message": f"Job id {job['id']} not found"}), 404
    except Exception as e:
        logging.error(f"Error triggering scrapper for job id {job['id']}: {e}")
        return jsonify({"message": f"Error triggering scrapper for job id {job['id']}"}), 500

@app.route("/trigger_scrapper/<int:id>", methods=["GET"])
def trigger_scrapper(id):
    """API to trigger the scrapper function based on job id from config.json"""
    try:
        with open('config.json', 'r') as config_file:
            config_data = json.load(config_file)['data']
        
        job = next((job for job in config_data if job['id'] == id), None)
        if job:
            scrapper(job['parameters'])
            logging.info(f"Scrapper triggered successfully for job id {id}.")
            return jsonify({"message": f"Scrapper triggered successfully for job id {id}"}), 200
        else:
            logging.warning(f"Job id {id} not found in config.json.")
            return jsonify({"message": f"Job id {id} not found"}), 404
    except Exception as e:
        logging.error(f"Error triggering scrapper for job id {id}: {e}")
        return jsonify({"message": f"Error triggering scrapper for job id {id}"}), 500

class AppService(win32serviceutil.ServiceFramework):
    _svc_name_ = "LinkedInScrapperService"
    _svc_display_name_ = "LinkedIn Scrapper Service"
    _svc_description_ = "Service to run LinkedIn scrapper as a Windows service."
